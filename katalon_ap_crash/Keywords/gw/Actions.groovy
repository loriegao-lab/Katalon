...
public class Actions {
	...
	def void SelectOptionAndEnter(TestObject to, String option) {
		if (option == null || option == "") return
			...
			try {
				...
				Thread.sleep(GlobalVariable.gWaitTimeOut)
	...
	def void NavigateByActions(String menuItems) {
		def actions = ObjectRepository.findTestObject("Common/OOTB/pge_Common/wel_Actions")
		//		Thread.sleep(GlobalVariable.gSleepTime)
		//		WebUI.click(actions)
		Click(actions)
		...
		/*String[] items = menuItems.split(';')
		 for (item in items) {
		 def menu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem', [('item') : item])
		 //			Thread.sleep(GlobalVariable.gSleepTime)
		 //			WebUI.click(menu)
		 Click(menu)
		 }*/
		...
		for (def i=0; i<len - 1; i++ ) {
			def menu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem', [('item') : items[i]])
			Thread.sleep(GlobalVariable.gSleepTime)
			WebUI.mouseOver(menu)
		}
		def lastmenu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem', [('item') : items[len-1]])
		Click(lastmenu)
	}
	...
	def void NavigateByClaimActions(String menuItems) {
		...
		def len = items.length
		for (def i=0; i<len - 1; i++ ) {
			def menu = ObjectRepository.findTestObject('ClaimCenter/OOTB/pge_Common/wel_MenuItem', [('item') : items[i]])
			Thread.sleep(GlobalVariable.gSleepTime)
			WebUI.mouseOver(menu)
		}
		def lastmenu = ObjectRepository.findTestObject('ClaimCenter/OOTB/pge_Common/wel_MenuItem', [('item') : items[len-1]])
		Click(lastmenu)
	}
	//	@Keyword
	//	def void NavigateBySideMenu(String menuItem) {
	//		def menu = ObjectRepository.findTestObject("Common/OOTB/pge_Common/wel_SideMenu", [('item') : menuItem])
	//		//		Thread.sleep(GlobalVariable.gSleepTime)
	//		//		WebUI.click(menu)
	//		Click(menu)
	//	}
	...
	def void NavigateByTabMenu(String tabName, String menuItems) {
		def tab = ObjectRepository.findTestObject("Common/OOTB/pge_Common/wel_TabBar", [('item') : tabName])
	...
	def void NavigateByTabMenus(String tabName, String menuItems) {
		...
		def tab = ObjectRepository.findTestObject("Common/OOTB/pge_Common/wel_TabToggle", [('item') : tabName])
		...
		if(!WebUI.verifyElementVisible(tab, FailureHandling.OPTIONAL)){
			Click(findTestObject('Common/OOTB/pge_Common/wel_TabsMoreOptions'))
			def menu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem', [('item') : tabName])
			WebUI.mouseOver(menu)
			Thread.sleep(GlobalVariable.gSleepTime)
		}
	...
	@Keyword
	def void NavigateByTabMenusNewChanges(String tabName, String menuItems) {
		...
		if(!WebUI.verifyElementVisible(tab, FailureHandling.OPTIONAL)){
			Click(findTestObject('Common/OOTB/pge_Common/wel_TabsMoreOptions'))
			def menu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem_New', [('item') : tabName])
			WebUI.mouseOver(menu)
			Thread.sleep(GlobalVariable.gSleepTime)
		}
		...
		def len = items.length
		for (def i=0; i<len - 1; i++ ) {
			def menu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem_New', [('item') : items[i]])
			Thread.sleep(GlobalVariable.gSleepTime)
			WebUI.mouseOver(menu)
		}
		def lastmenu = ObjectRepository.findTestObject('Common/OOTB/pge_Common/wel_MenuItem_New', [('item') : items[len-1]])
		Click(lastmenu)
	}
	...
	def void NavigateBySubTab(String tabName) {
		def tab = ObjectRepository.findTestObject("Common/OOTB/pge_Common/wel_SubTab", [('item') : tabName])
		//		WebUI.waitForElementClickable(tab,GlobalVariable.gWaitTimeOut)
		//		WebUI.click(tab)
		Click(tab)
	}
	...
	def void SelectRadioByLabelValue(String label,String value){
		...
		def tab = ObjectRepository.findTestObject('PolicyCenter/OOTB/Legacy/pge_NewSubmissions/wrg_Generic_Radio', [('label'): label, ('value') : value ] )
	...
	def void SelectTableItemByRefColumn(TestObject TableObject,String RefColHeader,String RefColValue,String ActionColHeader){
		...
		//def TableObject = ObjectRepository.findTestObject('PolicyCenter/OOTB/pge_AccountSummary/wtb_OpenTransactions')
		...
		table: for (int i = 0; i < Rows.size(); i++) {
			...
			COLSLoop:for (int j = 0; j < Cols.size(); j++) {
				...
				if (Cols.get(j).getText().equalsIgnoreCase(RefColValue)) {
					...
					println(Linkpath)
					Cols.get(ActionColumnIndex).findElement(By.xpath('./descendant::div[last()][last()]')).click()
					lblnClicked = true
	...
	//		TestObject tab = ObjectRepository.findTestObject("PolicyCenter/OOTB/pge_JobWizard/pge_Locations/btn_SelectLocation", [('Number') : LocationNumber])
	//		Click(tab)
	//	}
	...
	//			TestObject tab = ObjectRepository.findTestObject("PolicyCenter/OOTB/pge_JobWizard/pge_Payment/wel_Select", [('InstallmentPlan') : InstallmentPlan])
	//		Click(tab)
	//	}
	//
	//	@Keyword
