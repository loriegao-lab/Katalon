...
public class ExcelPlugIn {
	...
	static void setValueToCellByIndex(Sheet sheet, int rowIndex, int colIndex, def value) {
		...
		Cell cell = row.getCell(colIndex,  Row.MissingCellPolicy.CREATE_NULL_AS_BLANK)	// updated for Katalon 7.1.2:Row.MissingCellPolicy.CREATE_NULL_AS_BLANK
		setValueToCell(cell, value)
	}

	/**  
	 * Append value to cell by cell index  
	 *  
	 * @param sheet sheet in workbook  
	 * @param colIndex column index starts by 0  
	 * @param rowIndex row index starts by 0  
	 * @param value value to append with any data type (String, Numeric, Boolean, Date with format MM/dd/yyyy,  
	 *                 formula with character "=" at the beginning of the string  
	 * @Example appendValueToCellByIndex(sheet, 1, 2, 123)  
	 */  
	@Keyword
	static void appendValueToCellByIndex(Sheet sheet, int rowIndex, int colIndex, def value) {
		if (colIndex < 0 || rowIndex < 0) {
			KeywordUtil.logInfo("Column Index or Row Index should >= 0")
			return
		}
		...
		def existingValue = cell.toString()
		def newValue = ''
		if (existingValue == '') {
	...
	/**
	 * Write Automation Compare Result into Excel
	 * Invoked by 'CompareXXX.groovy' methods
	 * @XlsTab is according to LOB scenarios.
	 * @resultList is the result after comparing with DB base line policy
	 * @PolicyNumber is the latest NewSubmission policy need to be compared
	 */
	def static void writeCompareResult(String XlsTab, ArrayList resultList, String PolicyNumber, String SubmissionNumber, String Lob){
		Timestamp time = new Timestamp(System.currentTimeMillis())
		String timestamp = time.toString().substring(0,10).replace('-','')
		String f_Path = '.\\\\..\\\\TestData\\\\PC\\\\PC_Output'
		String compareFileName = f_Path+'.xlsx'
		FileInputStream file = new FileInputStream(new File(compareFileName))
		XSSFWorkbook workbook = new XSSFWorkbook(file)
		XSSFSheet sumSheet = workbook.getSheet('EDW')
		int rowCount = sumSheet.getLastRowNum()
		XSSFSheet sheet
		try {
			/*if(workbook.getSheet(XlsTab)==null){
			 sheet = workbook.createSheet(XlsTab)
			 }*/
			sumSheet.createRow(rowCount+1).createCell(3).setCellValue(XlsTab)
			sumSheet.getRow(rowCount+1).createCell(0).setCellValue(Lob)
			sumSheet.getRow(rowCount+1).createCell(1).setCellValue(PolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(2).setCellValue(SubmissionNumber)
			if(resultList.size == 0){
				sumSheet.getRow(rowCount+1).createCell(4).setCellValue(true)
				for(int i = 0; i < resultList.size; i++){
					sumSheet.getRow(rowCount+1).createCell(5).setCellValue(resultList[i].toString())
				}
			}
			else{
				sumSheet.getRow(rowCount+1).createCell(4).setCellValue(false)
				for(int i = 0; i < resultList.size; i++){
					sumSheet.getRow(rowCount+1).createCell(5).setCellValue(resultList[i].toString())
				}
				//sheet = workbook.getSheet(XlsTab)
			}
			/*	for(int i = 0; i < resultList.size; i++){
			 int j = 0
			 sheet.createRow(i+1).createCell(j).setCellValue(i+1)
			 println ('result list:'+i)
			 println(resultList[i])
			 resultList[i].each{
			 //sheet.getRow(i+1).createCell(j+1).setCellValue(resultList[i][j].toString())
			 sheet.getRow(i+1).createCell(j+1).setCellValue(resultList[i].toString())
			 j++
			 }
			 }*/
			...
			file.close()
			FileOutputStream outFile =new FileOutputStream(new File(compareFileName))
			workbook.write(outFile)
			outFile.close()
		}
	}
	////////////////****************************************************EDW New Submission*********************************************************************////////////////////
	def static void writeCompareResult_Compare(String XlsTab, ArrayList resultListSIT,ArrayList resultListPRE,String PREPolicyNumber,String PRESubmissionNumber, String PolicyNumber, String SubmissionNumber, String Lob) {
		Timestamp time = new Timestamp(System.currentTimeMillis())
		String timestamp = time.toString().substring(0,10).replace('-','')
		String f_Path = '.\\\\..\\\\TestData\\\\PC\\\\PC_Data_Compare_Output'
		String compareFileName = f_Path+'.xlsx'
		FileInputStream file = new FileInputStream(new File(compareFileName))
		XSSFWorkbook workbook = new XSSFWorkbook(file)
		XSSFSheet sumSheet = workbook.getSheet('EDW')
		...
		XSSFSheet sheet
		try {
			sumSheet.createRow(rowCount+1).createCell(5).setCellValue(XlsTab)
			sumSheet.getRow(rowCount+1).createCell(0).setCellValue(Lob)
			sumSheet.getRow(rowCount+1).createCell(1).setCellValue(PREPolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(2).setCellValue(PRESubmissionNumber)
			sumSheet.getRow(rowCount+1).createCell(3).setCellValue(PolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(4).setCellValue(SubmissionNumber)

			if(resultListSIT.size != 0 && resultListPRE.size != 0) {
				if(resultListSIT.equals(resultListPRE)) {

					sumSheet.getRow(rowCount+1).createCell(6).setCellValue(true)
					for(int i = 0; i < resultListSIT.size; i++) {
						sumSheet.getRow(rowCount+1).createCell(7).setCellValue(resultListSIT[i].toString())
					}
				}
				else {
					sumSheet.getRow(rowCount+1).createCell(6).setCellValue(false)
					for(int i = 0; i < resultListSIT.size; i++) {
						sumSheet.getRow(rowCount+1).createCell(7).setCellValue(resultListSIT[i].toString())
					}
				}
	...
	////////////////*****************************************EDW Policy Change**************************************************************////////////////////
	def static void writeCompareResult_Compare_Policy_Change(String XlsTab, ArrayList resultListSIT,ArrayList resultListPRE,String PREPolicyNumber,String PRESubmissionNumber, String PolicyNumber, String SubmissionNumber, String Lob) {
		...
		FileInputStream file = new FileInputStream(new File(compareFileName))
		XSSFWorkbook workbook = new XSSFWorkbook(file)
		XSSFSheet sumSheet = workbook.getSheet('EDW Policy Change')
		int rowCount = sumSheet.getLastRowNum()
		XSSFSheet sheet
		try {
			sumSheet.createRow(rowCount+1).createCell(5).setCellValue(XlsTab)
	...
	////////////////*****************************************EDW Full Term Cancellation**************************************************************////////////////////
	def static void writeCompareResult_Compare_FullTerm_Cancellation(String XlsTab, ArrayList resultListSIT,ArrayList resultListPRE,String PREPolicyNumber,String PRESubmissionNumber, String PolicyNumber, String SubmissionNumber, String Lob) {
		...
		FileInputStream file = new FileInputStream(new File(compareFileName))
		XSSFWorkbook workbook = new XSSFWorkbook(file)
		XSSFSheet sumSheet = workbook.getSheet('EDW Cancellation')
		int rowCount = sumSheet.getLastRowNum()
		XSSFSheet sheet
		try {
			sumSheet.createRow(rowCount+1).createCell(7).setCellValue(XlsTab)
			sumSheet.getRow(rowCount+1).createCell(0).setCellValue(Lob)
			sumSheet.getRow(rowCount+1).createCell(1).setCellValue(PREPolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(2).setCellValue(PRESubmissionNumber)
			sumSheet.getRow(rowCount+1).createCell(4).setCellValue(PolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(5).setCellValue(SubmissionNumber)

			if(resultListSIT.size != 0 && resultListPRE.size != 0) {
				if(resultListSIT.equals(resultListPRE)) {

					sumSheet.getRow(rowCount+1).createCell(8).setCellValue(true)
					for(int i = 0; i < resultListSIT.size; i++) {
						sumSheet.getRow(rowCount+1).createCell(9).setCellValue(resultListSIT[i].toString())
					}
				}
				else {
					sumSheet.getRow(rowCount+1).createCell(8).setCellValue(false)
					for(int i = 0; i < resultListSIT.size; i++) {
						sumSheet.getRow(rowCount+1).createCell(97).setCellValue(resultListSIT[i].toString())
					}
				}
			}
	...
	////////////////*****************************************EDW Mid Term Cancellation**************************************************************////////////////////
	def static void writeCompareResult_Compare_MidTerm_Cancellation(String XlsTab, ArrayList resultListSIT,ArrayList resultListPRE,String PREPolicyNumber,String PRESubmissionNumber, String PolicyNumber, String SubmissionNumber, String Lob) {
		...
		try {
			...
			sumSheet.getRow(rowCount+1).createCell(0).setCellValue(Lob)
			sumSheet.getRow(rowCount+1).createCell(1).setCellValue(PREPolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(3).setCellValue(PRESubmissionNumber)
			sumSheet.getRow(rowCount+1).createCell(4).setCellValue(PolicyNumber)
			sumSheet.getRow(rowCount+1).createCell(6).setCellValue(SubmissionNumber)
	...
	////////////////****************************************************EDW Reinstate*********************************************************************////////////////////
	def static void writeCompareResult_Reinstate(String XlsTab, ArrayList resultListSIT,ArrayList resultListPRE,String PREPolicyNumber,String PRESubmissionNumber, String PolicyNumber, String SubmissionNumber, String Lob) {
		Timestamp time = new Timestamp(System.currentTimeMillis())
		String timestamp = time.toString().substring(0,10).replace('-','')
		String f_Path = '.\\\\..\\\\TestData\\\\PC\\\\PC_Data_Compare_Output'
		String compareFileName = f_Path+'.xlsx'
		FileInputStream file = new FileInputStream(new File(compareFileName))
		XSSFWorkbook workbook = new XSSFWorkbook(file)
		XSSFSheet sumSheet = workbook.getSheet('EDW Reinstate')
		int rowCount = sumSheet.getLastRowNum()
		XSSFSheet sheet
		try {
			sumSheet.createRow(rowCount+1).createCell(5).setCellValue(XlsTab)
	...
	/**
	 * Design CompareResult spreadsheet headline column by EDW table.
	 * Invoked by function 'writeCompareResult'
	 */
	private static void writeheadLine(XSSFSheet sheet, ArrayList resultList){
		String resultLine = resultList[0].toString()
		resultLine = resultLine.substring(1, resultLine.length()-1)
		String[] resultCols = resultLine.split(",")
		sheet.createRow(0)
		String colName = ''
		for(int k = 0; k<=resultCols.size()-1;k++){
			colName = resultCols[k].toString().split("=")[0].trim()
			sheet.getRow(0).createCell(k+1).setCellValue(colName)
		}
	}
	...
	/**
	 * Get row index list by cell content
	 *
	 * @param sheet the sheet object that contains the row
	 * @param cellContent the content of the cell
	 * @param columnIdxForCell the column index of the cell
	...
	@CompileStatic
	@Keyword
	static List<Integer> getRowIndexListByCellContent(Sheet sheet, String cellContent, int columnIdxForCell){
		...
		for(Row row: sheet){
			...
			if (null != cell){
				boolean match = getCellValue(cell).toString().trim() == cellContent
				if(match){
					rowIndexes.add(cell.getRowIndex())
				}
			}
		}
		return rowIndexes
